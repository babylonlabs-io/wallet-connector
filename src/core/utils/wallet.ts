...
export function validateAddress(network: Network, address: string): void {
  if ([Network.MAINNET, Network.CANARY].includes(network) && !address.startsWith("bc1")) {
    throw new Error("Incorrect address prefix for Mainnet/Canary. Expected address to start with 'bc1'.");
  ...
  } else if (![Network.MAINNET, Network.SIGNET, Network.TESTNET, Network.CANARY].includes(network)) {
